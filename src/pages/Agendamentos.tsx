...
import { format, parseISO, startOfWeek, endOfWeek, startOfMonth, endOfMonth, isWithinInterval, subMonths, isAfter, isBefore, startOfDay, endOfDay } from "date-fns";
import { DateRange } from "react-day-picker";
import { ptBR } from "date-fns/locale";
import { CalendarDays, Plus, Settings2, List, BarChart3, Eye, EyeOff, Search, ChevronDown, Clock, MapPin, Video, Phone, MessageSquare, Edit2, RefreshCw, Check, X, Loader2, Trash2, TrendingUp, TrendingDown, Users, CheckCircle2, XCircle, AlertCircle } from "lucide-react";
...
export default function Agendamentos() {
  ...
  // Agendamentos filtrados por dateRange
  ...
  // Métricas calculadas a partir dos filtrados (respeitam dateRange)
  ...
  // Calendar events (sem filtro de data — o calendário tem navegação própria)
  ...
  // Métricas por semana (últimas 8 semanas)
  ...
  // Comparecimento mensal (últimos 6 meses)
  ...
  return (
    <div className="space-y-8 max-w-full overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-foreground">Agendamentos</h1>
          <p className="text-sm text-muted-foreground mt-1">Gerencie reuniões, consultas e acompanhe resultados</p>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <Button variant="outline" size="sm" className="gap-1.5 text-xs" onClick={() => setModalConfig(true)}>
            <Settings2 className="h-3.5 w-3.5" /> Notificações
          </Button>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="lista" className="mt-6 space-y-6">
          ...
          {/* Métricas rápidas — Dashboard-style cards */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: "Total", value: agendamentosFiltrados.length, icon: CalendarDays, color: "hsl(var(--primary))" },
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative flex-1 max-w-sm">
              ...
              <Input placeholder="Buscar por lead ou título..." value={filtroBusca} onChange={(e) => setFiltroBusca(e.target.value)} className="pl-9 h-9 text-xs" />
            </div>
          </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/30">
                    <th className="text-left px-4 py-3 text-[10px] font-semibold text-muted-foreground uppercase tracking-widest">Lead</th>
                    ...
                    <th className="text-left px-4 py-3 text-[10px] font-semibold text-muted-foreground uppercase tracking-widest">Resultado</th>
                ...
                <tbody className="divide-y divide-border/50">
                  {listaFiltrada.map((ag) => {
                    const TipoIcon = TIPO_ICONS[ag.tipo] || CalendarDays;
                    return (
                      <tr key={ag.id} className="hover:bg-muted/20 transition-colors cursor-pointer group" onClick={() => setExpandedRow(expandedRow === ag.id ? null : ag.id)}>
                        <td className="px-4 py-3.5 font-medium text-foreground">{ag.lead?.nome || "—"}</td>
                        ...
                        <td className="px-4 py-3.5">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent>
                              {Object.entries(STATUS_LABELS).map(([k, v]) => (
                                <DropdownMenuItem key={k} onClick={(e) => { e.stopPropagation(); handleStatusChange(ag, k); }}>
                        ...
                        <td className="px-4 py-3.5 text-xs text-muted-foreground">{ag.resultado || "—"}</td>
                        <td className="px-4 py-3.5 text-right">
                          <div className="flex items-center justify-end gap-0.5 opacity-60 group-hover:opacity-100 transition-opacity">
                            <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={(e) => { e.stopPropagation(); openDetalhes(ag); }}>
                              <Eye className="h-3.5 w-3.5" />
                            </Button>
                            <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={(e) => { e.stopPropagation(); openEditar(ag); }}>
                              <Edit2 className="h-3.5 w-3.5" />
                            </Button>
                            <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-red-500 hover:text-red-700" onClick={(e) => {
                              e.stopPropagation();
                              setAgendamentoExcluir(ag);
                            }}>
        ...
        <TabsContent value="metricas" className="mt-6 space-y-8">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: "Total Agendados", value: String(metricasFiltradas.total), icon: CalendarDays, color: "hsl(var(--primary))" },
                  ...
                  {tendenciaSemanal.length > 0 ? (
                    <ResponsiveContainer width="100%" height={250}>
                      <LineChart data={tendenciaSemanal}>
                        ...
                        <RechartsTooltip contentStyle={{ borderRadius: '8px', border: '1px solid hsl(var(--border))', fontSize: '12px' }} />
                        <Line type="monotone" dataKey="total" stroke="hsl(var(--primary))" strokeWidth={2.5} dot={{ r: 4, fill: 'hsl(var(--primary))', strokeWidth: 2, stroke: 'hsl(var(--background))' }} />
                      </LineChart>
                    </ResponsiveContainer>
                  ) : (
                    <div className="flex flex-col items-center justify-center py-12 text-muted-foreground/50">
      ...
      <Dialog open={modalCriar} onOpenChange={(o) => { if (!o) { setModalCriar(false); resetForm(); } }}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editMode ? "Editar Agendamento" : "Novo Agendamento"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <Label>Lead</Label>
              <Select value={form.lead_id || ""} onValueChange={(v) => {
                const lead = leadsOrg.find((l) => l.id === v);
                setForm((f) => ({ ...f, lead_id: v, titulo: f.titulo || `Reunião - ${lead?.nome || ""}` }));
              }}>
                <SelectTrigger><SelectValue placeholder="Selecione um lead..." /></SelectTrigger>
                <SelectContent>
                  {leadsOrg.map((l) => <SelectItem key={l.id} value={l.id}>{l.nome}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div>
              <Label>Título</Label>
              <Input value={form.titulo} onChange={(e) => setForm((f) => ({ ...f, titulo: e.target.value }))} placeholder="Ex: Reunião - João Silva" />
            </div>
            ...
            {!editMode && (
              <div className="flex items-center gap-2">
                ...
                <Label htmlFor="enviar-confirm" className="cursor-pointer">Enviar confirmação via WhatsApp agora</Label>
              </div>
            )}
          </div>
          <DialogFooter>
          ...
          {agendamentoSelecionado && (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div><span className="text-muted-foreground">Lead:</span> {agendamentoSelecionado.lead?.nome || "—"}</div>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Label htmlFor="lead-avancou" className="cursor-pointer">Lead avançou no pipeline?</Label>
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={modalReagendar} onOpenChange={setModalReagendar}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleReagendar} disabled={formLoading}>
              {formLoading && <Loader2 className="h-4 w-4 mr-1 animate-spin" />}
              Reagendar
            </Button>
          </DialogFooter>
